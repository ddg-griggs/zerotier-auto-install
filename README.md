...
ZeroTier успешно настроен (MTU сервера зафиксирован на 1280).
Ваш IP в сети ZeroTier: $(ip -4 addr show $ZT_IFACE | grep -oP '(?<=inet\s)\d+(\.\d+){3}')

Что нужно сделать сейчас в панели my.zerotier.com:
1. Поставьте галочку Auth напротив этого сервера.
2. В разделе Routes добавьте маршрут: 0.0.0.0/0 через IP этого сервера (в колонке Managed IPs).
3. В разделе DNS укажите:
   - Server Address: 8.8.8.8 и 1.1.1.1
   - Search Domain: local или . (точка) (если панель требует заполнения этого поля)

На вашем ТЕЛЕФОНЕ:"
1. Включите галочку 'Default Route' (Route via ZeroTier).
2. Наслаждайтесь работающим YouTube и Gemini!
